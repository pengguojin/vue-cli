...
<div class="datepicker" :class="{'datepicker-range':range,'datepicker__clearable':clearable&&text&&!disabled}">
  <b-form-input :value="text" :class="[show ? 'focus' : 'outfocus']" :readonly="readonly" :disabled="disabled" :placeholder="placeholder" autoComplete="off" :name="name" v-if="type!=='inline'&&invalid" :state="state" @change="handelChange"/>
  <b-form-input :value="text" :class="[show ? 'focus' : 'outfocus']" :readonly="readonly" :disabled="disabled" :placeholder="placeholder" autoComplete="off" :name="name" v-if="type!=='inline'&&!invalid" @change="handelChange"/>
...
export default {
  ...
  methods: {
    ...
    handelChange (val){ //add by pgj 修改手输不能没有生效的bug
